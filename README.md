<div align="center">
  ...
  ![版本](https://img.shields.io/badge/版本-v2.0-2b6cb0) ![动画](https://img.shields.io/badge/算法动画-43个-2f855a) ![覆盖](https://img.shields.io/badge/覆盖-全部8章·对齐408考纲-c05621) ![测试](https://img.shields.io/badge/正确性断言-281项通过-553c9a) ![依赖](https://img.shields.io/badge/依赖-零·单文件-4a5568)
...
**第2章 线性表**：② 顺序表插入/删除 ③ 单链表插入/删除 ④ 顺序表基本操作合集 ⑤ 链表基本操作合集 ⑥ 合并有序表 ⑦ 双向/循环链表 ⑧ 多项式相加
**第3章 栈和队列**：⑨ 顺序栈 ⑩ 假溢出与循环队列（四方案） ⑪ 递归调用栈·汉诺塔 ⑫ 括号匹配 ⑬ 表达式求值（双栈法） ⑭ 数制转换 ⑮ 迷宫求解
**第4章 串和数组**：⑯ 模式匹配 BF/KMP（next 数组） ⑰ 矩阵压缩存储（对称映射 + 快速转置）
**第5章 树和二叉树**：⑱ 四种遍历 ⑲ 中序线索二叉树 ⑳ 哈夫曼树与编码/译码 ㉑ 树/森林与二叉树转换 ㉒ 堆的建立与调整
...
`tests/test.js` 内置 **281 项断言**（GitHub Actions 每次推送自动执行），与教材/PPT 例题逐一对拍，例如：
