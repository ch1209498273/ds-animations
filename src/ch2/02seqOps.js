...
(function () {
  ...
  var CODE = [
    ...
    '    return 0;                             // 未找到',
    '}',
    'ElemType GetElem(SqList L, int i) {       // 取值',
    '    if (i < 1 || i > L.length) return ERROR;   // 位序从 1 起算',
    '    return L.elem[i-1];                   // 下标 = 位序 − 1，一次寻址 O(1)',
    '}',
    'int Length(SqList L) { return L.length; } // 求表长：读现成字段 O(1)',
    'void Traverse(SqList L, void (*visit)(ElemType)) {   // 遍历',
    '    for (i = 0; i < L.length; ++i)  visit(L.elem[i]);',
    '}',
    'int MaxElem(SqList L) {                   // 求最大值',
    '    max = 0;                              // 先记下标 0 作为候选',
    '    for (i = 1; i < L.length; ++i)',
    '        if (L.elem[i] > L.elem[max])  max = i;',
    '    return max;',
    '}'
  ];
  ...
  DSC.reg({
    ...
    aim: '按位查、按值查、插、删、求表长一套走完，看清**平均要移动一半元素**',
    note: '教材 2.4 基本操作（查找/取值/表长/最值，随机存取特性）',
    keywords: '查找 LocateElem 取值 GetElem 表长 Length 最大元素 时间性能 位序 下标 越界 随机存取 O(1)',
    guide: [
    ...
    run: function (v) {
      ...
      F([], '顺序表 L =（' + arr.join(', ') + '），n = ' + n + '。位序 i 从 1 起，下标 = i−1。', {});
      ...
      if (op === 'find') {
        ...
        F([0, 1, 2, 3], '★ 小结：按值查找 O(n)。等概率成功 ASL=(n+1)/2=' + ((n + 1) / 2).toFixed(1) + '。想 O(1) 查找 → 第 7 章散列表。', { hit: found >= 0 ? found : undefined, res: found >= 0 ? '位序 ' + (found + 1) : '未找到' });
      } else if (op === 'get') {
        if (pos < 1 || pos > n) {
          F([6], '先判合法：位序必须满足 1 ≤ i ≤ L.length = ' + n + '。这次给的是 i = ' + pos + ' → 越界。', { res: '…' });
          F([6], '★ 返回 ERROR。顺序表"按下标直达"的前提是下标真的落在数组里，越界没有任何补救办法。', { res: 'ERROR：i 越界' }, 'err');
        } else {
          F([7], '取第 ' + pos + ' 个元素：位序 i 映射下标 **i−1**，所以要读的是 elem[' + (pos - 1) + ']。' +
            '这一步只是算一个地址，不比较、不移动。', { res: '…' });
          F([7], 'elem[' + (pos - 1) + '] = ' + arr[pos - 1] + '。位序 ' + pos + ' 直接映射下标 ' + (pos - 1) + '，**一次寻址 O(1)**，与 n 无关。', { hit: pos - 1, res: '第 ' + pos + ' 个 = ' + arr[pos - 1] }, 'ok');
          F([5, 7], '★ 这就是顺序表被称为"随机存取结构"的原因：取任何一位都是 O(1)。' +
            '换成单链表取第 ' + pos + ' 个，得从头顺着 next 走 ' + (pos - 1) + ' 步，O(n)。', { hit: pos - 1, res: '第 ' + pos + ' 个 = ' + arr[pos - 1] }, 'ok');
        }
      } else if (op === 'len') {
        F([9], '顺序表的定义里带了一个 `length` 字段，每次插入/删除都顺手改过它——' +
          '所以表长不是"数出来的"，是**记下来的**。', { res: '…' });
        F([9], '读 L.length 就是读一个整数：n = ' + n + '，**O(1) 直接返回**。',
          { done: arr.map(function (_, i) { return i; }), res: 'n = ' + n }, 'ok');
        F([9], '★ 对照：单链表没有这个字段，求表长必须从头顺着 next 数一遍 O(n)。' +
          '顺序表在"读长度、读第 i 个"上都是 O(1)，代价全在插入删除的元素移动上。',
          { done: arr.map(function (_, i) { return i; }), res: 'n = ' + n }, 'ok');
      } else if (op === 'max') {
        var mx = 0;
        F([14], '候选先记**下标** 0（不是值），后面每来一个更大的就把候选换成它的下标。', { m: 0 });
        for (var i2 = 1; i2 < n; i2++) {
          cmp++;
          if (arr[i2] > arr[mx]) { mx = i2; F([16], 'elem[' + i2 + ']=' + arr[i2] + ' > 当前最大 elem[' + (mx === i2 ? i2 : mx) + '] → 更新最大值候选。', { m: i2, c: i2 }); }
          else F([16], 'elem[' + i2 + ']=' + arr[i2] + ' ≤ 当前最大 ' + arr[mx] + '，不更新。', { m: mx, c: i2 });
        }
        F([13, 17], '★ 最大值 = elem[' + mx + '] 位置的 ' + arr[mx] + '（位序 ' + (mx + 1) + '），共比较 ' + (n - 1) + ' 次。' +
          '求最值必须扫完全表，O(n) 没有捷径。', { m: mx, res: 'max = ' + arr[mx] }, 'ok');
      } else {
        var out = [];
        for (var i3 = 0; i3 < n; i3++) {
          out.push(arr[i3]);
          F([11], '第 ' + (i3 + 1) + ' 轮：visit(elem[' + i3 + ']) 输出 ' + arr[i3] + '（已输出 ' + out.join(', ') + '）', { done: out.map(function (_, j) { return j; }) });
        }
        F([10, 11], '★ 遍历完成：L =（' + out.join(', ') + '）。' +
          '下标 0 → n−1 顺序扫一遍，O(n)；顺序表的遍历**不需要任何辅助指针**，这是它比链表省事的地方。',
          { done: out.map(function (_, j) { return j; }), res: '共 ' + n + ' 个' }, 'ok');
      }
