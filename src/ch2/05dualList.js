...
(function () {
  ...
  DSC.reg({
    ...
    keywords: '双向链表 prior 对称改链 四个指针 循环链表 单循环 双向循环 rear 判空 判尾 next==head 从表尾回溯',
    guide: [
    ...
    run: function (v) {
      ...
      if (op === 'bad') {
        F('错误演示：把正确顺序 ①②③④ 颠倒成 **④①②③**。初始状态正常，s（蓝虚线框）待接入，p 指向 30。', { stage: 'start', p: 30, sFloat: 25 }, 'bad0', []);
        ...
        F('① s->prior = p->prior：而 p->prior 已经是 s 自己 → **s->prior 指向 s，形成自环**！前驱 20 的信息永久丢失。', { stage: 'w2', p: 30, sFloat: 25 }, 'bad2', 0);
        F('② p->prior->next = s：p->prior 是 s → s->next = s，**next 也自环了**。正向链 10→20→30→40 看似完好——隐患藏在反向。', { stage: 'w3', p: 30, sFloat: 25 }, 'bad3', 1);
        F('③ s->next = p：s 接上了 30。表面看四步都执行了……', { stage: 'w4', p: 30, sFloat: 25 }, 'bad4', 2);
        F('后果检查：从 40 沿 prior 回走：40 → 30 → s → s → s…… **死循环**！20 和 10 从反向链上永远消失；这个"链表"只有正向能用，反向已坏——这就是颠倒顺序的代价。', { stage: 'w5', sFloat: 25 }, 'bad5', []);
        F('结论：四步的正确顺序本质是"**先用 ① s->prior 备份前驱，再用 ② 改前驱的 next**"。任何先改后备份的顺序都会丢信息。对照正确场景再走一遍。', { stage: 'w5' }, 'done', [0, 1]);
        return { code: CODE_I, frames: frames };
      }
      if (op === 'del') {
