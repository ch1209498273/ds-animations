    ...
    render: function (s) {
      ...
      if (s.storage === 'list') {
        ...
        for (var li = 1; li <= N; li++) {
          ...
          g += h.txt(mx - 8, ly + 22, 'v' + li, { size: 13.5, w: 700, anchor: 'end' });
          g += h.txt(mx + 2, ly + 22, '→', { size: 13, fill: C.muted });
          var nb = s.adj[li] || [];
        ...
        g += h.txt(mx + 3 * cell, my - 22, '邻接矩阵', { size: 14, w: 600 });
        g += h.txt(mx - 14, my + 6, ' ', { size: 10 });
        for (var j = 1; j <= N; j++) g += h.txt(mx + (j - 0.5) * cell, my + 6, String(j), { size: 11, fill: C.muted });
        for (var a = 1; a <= N; a++) {
          g += h.txt(mx - 12, my + (a - 0.5) * cell + 14, String(a), { size: 11, fill: C.muted });
          for (var b = 1; b <= N; b++) {
            var v = s.mat[a - 1][b - 1];
